...
import { createCommunityCase } from '@/services/firebase/communityFirestore';
...
export function CreateCasePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Upload all images
      const uploadedImages: CommunityCaseImage[] = await Promise.all(
      ...
      // Create case
      const caseId = await createCommunityCase(user.uid, user.displayName, {
        title: title.trim(),
        description: description.trim(),
        images: uploadedImages,
        diagnosis: includeDiagnosis && diagnosisText.trim()
          ? {
              text: diagnosisText.trim(),
              histopathologyResult: histopathology.trim() || undefined,
            }
          : undefined,
      });
    ...
    <Layout>
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-charcoal">{t('community.createCase')}</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Images */}
          <Card>
            <h2 className="font-semibold text-charcoal mb-4">{t('community.images')}</h2>
            ...
            <label className="block border-2 border-dashed border-gray-300 rounded-lg p-8 text-center cursor-pointer hover:border-primary hover:bg-primary/5 transition-colors mb-4">
              ...
              <p className="text-sm text-gray-500 mt-1">{t('community.imageTypes')}</p>
            </label>
